...
var awards = {
	...
	var icon_64 = get_icon(award);
        if ($("." + award[0] + "-awards").size() === 0 && location.href.indexOf('/profile/' + award[0] + '/') !== -1) {
            $("td.c_user").parent().after('<tr><td class="c_user"><dl class="user_profile" style="padding-bottom: 30px;"><dt>' + profile_zoo.name + ':</dt></td><td><dd style="margin: 0;" class="' + award[0] + '-awards"><div onmouseover="awards.tooltip.open(event,' + a + ');" onmouseout="awards.tooltip.bye(' + a + ');" id="' + a + '-award" class="' + icon_64 + '" alt="' + award[1] + '"></div></dd></dl></td></tr>');
    ...
    tooltip: {
	    ...
	    var icon_64 = get_icon(award);
	    if (icon_64.includes("-egg")) {
	        award[2] = "Egg";
	    }
            awards.tooltip.coords = [pos[0], pos[1]];
            if ($("#" + a + "-tooltip").size() === 0) $("body").append('<div id="' + a + '-tooltip" style="position:absolute;max-width:500px;"><table><tbody><tr><td style="padding: 0px;"><div class="' + icon_64 + '" alt="' + award[1] + '"></div></td><td>' + award[3] + '<hr /><b>' + award[1] + '</b> | ' + award[2] + '<hr /><i> ' + award[7] + '</i></td></tr></tbody></table></div>');
